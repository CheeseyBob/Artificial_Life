...
class Plant_Seasonal extends WorldObject implements Stepable {
	...
	public static Plant_Seasonal load(Scanner scanner, String pathname) {
		String[] data = scanner.next().split(":");
		boolean fruitsInSummer = Boolean.parseBoolean(data[0]);
		int stepsToBearFruit = Integer.parseInt(data[1]);
		return new Plant_Seasonal(fruitsInSummer, stepsToBearFruit);
	}
	
	Plant_Seasonal(boolean fruitsInSummer){
		this.fruitsInSummer = fruitsInSummer;
	}
	
	private Plant_Seasonal(boolean fruitsInSummer, int stepsToBearFruit){
		this.fruitsInSummer = fruitsInSummer;
